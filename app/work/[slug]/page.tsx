...
import { ArrowLeft, CheckCircle2, Layout, Zap, ShieldCheck } from 'lucide-react'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const cleanDesc = `${project.description} Re-imagined and engineered for high-performance by Lumora Triad.`
    ...
    <main className="min-h-screen bg-background">
      ...
      <nav className="fixed top-24 left-0 right-0 z-40 bg-background/80 backdrop-blur-md py-4 border-b border-border">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between">
          <Link 
            href="/work" 
            className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest hover:opacity-60 transition-opacity text-foreground"
          >
            <ArrowLeft size={14} /> Back to Work
          </Link>
          <div className="flex items-center gap-3">
             <span className="text-[10px] font-black uppercase tracking-widest opacity-40 text-foreground">{project.category}</span>
             <span className="w-1 h-1 rounded-full bg-foreground/20" />
             <span className="text-[10px] font-black uppercase tracking-widest opacity-40 text-foreground">{project.year}</span>
          </div>
        </div>
      </nav>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-24 items-start">
            <div className="lg:col-span-8 space-y-8">
              <h1 className="text-5xl lg:text-[7rem] font-bold leading-[0.9] tracking-tighter uppercase mb-6 text-foreground">
                {project.title}
              </h1>
              ...
              <div className="grid grid-cols-1 gap-10">
                {project.stats.map((stat) => (
                  <div key={stat.label} className="border-t border-border pt-6">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest opacity-40 text-foreground">{stat.label}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="py-24 px-6 lg:px-8 border-t border-border">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-3 gap-16 lg:gap-32">
            <div>
              <h2 className="text-xs font-black uppercase tracking-[0.2em] opacity-40 mb-8 inline-flex items-center gap-2 text-foreground">
                <Layout size={14} /> The Challenge
              </h2>
              <p className="text-xl font-medium leading-relaxed text-muted-foreground">{project.challenge}</p>
            </div>
            <div>
              <h2 className="text-xs font-black uppercase tracking-[0.2em] opacity-40 mb-8 inline-flex items-center gap-2 text-foreground">
                <Zap size={14} /> Our Solution
              </h2>
              <p className="text-xl font-medium leading-relaxed text-muted-foreground">{project.solution}</p>
            </div>
            <div>
              <h2 className="text-xs font-black uppercase tracking-[0.2em] opacity-40 mb-8 inline-flex items-center gap-2 text-foreground">
              ...
              <ul className="space-y-4">
                {project.fullDetails.map((detail, i) => (
                  <li key={i} className="flex items-start gap-3">
                    ...
                    <span className="text-sm font-bold opacity-80 text-foreground">{detail}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section className="py-24 px-6 lg:px-8 bg-foreground text-background">
        <div className="max-w-4xl mx-auto text-center">
          <ShieldCheck size={48} className="mx-auto mb-8 opacity-40 text-background" />
          <h2 className="text-4xl lg:text-7xl font-bold leading-tight mb-8 tracking-tighter text-background">
            Ready to Build Your Legacy?
          </h2>
          <p className="text-xl opacity-70 mb-12 max-w-2xl mx-auto text-background">
