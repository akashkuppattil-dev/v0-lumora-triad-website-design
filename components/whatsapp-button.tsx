...
export function WhatsAppButton() {
  ...
  return (
    <div
      ref={menuRef}
      className={`fixed bottom-6 right-4 sm:right-6 z-[100] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${isVisible ? 'translate-y-0 opacity-100 pointer-events-auto' : 'translate-y-16 opacity-0 pointer-events-none'
        }`}
    >
      {/* WhatsApp Chat Widget */}
      <div
        className={`absolute bottom-full right-0 mb-3 w-[300px] sm:w-[320px] rounded-2xl overflow-hidden shadow-2xl transition-all duration-500 origin-bottom-right ${isExpanded ? 'scale-100 opacity-100' : 'scale-90 opacity-0 pointer-events-none'
          }`}
      ...
      >
        <div className="relative w-7 h-7 flex items-center justify-center">
          <MessageCircle
            className={`absolute w-full h-full text-white transition-all duration-400 ${isExpanded ? 'scale-0 opacity-0' : 'scale-100 opacity-100'
              }`}
          />
          <X
            className={`absolute w-full h-full text-white transition-all duration-400 ${!isExpanded ? 'scale-0 opacity-0' : 'scale-100 opacity-100'
